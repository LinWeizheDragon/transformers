...
class FLMRTokenizerFast(BertTokenizerFast):
    r"""
    Constructs a "fast" FLMRQuestionEncoder tokenizer (backed by HuggingFace's *tokenizers* library).
...
class FLMRReaderTokenizerFast(CustomFLMRReaderTokenizerMixin, BertTokenizerFast):
    r"""
    Constructs a "fast" FLMRReader tokenizer (backed by HuggingFace's *tokenizers* library).

    [`FLMRReaderTokenizerFast`] is almost identical to [`BertTokenizerFast`] and runs end-to-end tokenization:
    punctuation splitting and wordpiece. The difference is that is has three inputs strings: question, titles and texts
    that are combined to be fed to the [`FLMRReader`] model.
    ...
    slow_tokenizer_class = FLMRReaderTokenizer
